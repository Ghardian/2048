...
public class NormalModeController implements IGameController {
    ...
    public Bitmap onDrawingRequested() {
        ...
        for(int i = 0; i < 4; i++)
        {
            for (int j = 0; j < 4; j++)
            {
                ...
                if(value!=0)
                {
                    ...
                    {
                        graphics.drawText(""+value, (side*j)-offsetY, (side*(i+offsetY)+side*((offsetX-offsetY)/offsetX)),fontSizeMedium);
        ...
        graphics.drawText("Join the numbers and get to the 2048 tile!",offsetXText,offsetYText,fontSizeSmall);
        graphics.drawText("Current Score: "+ mechanics.getScore(),(width-(width/2)+offsetXText),side,fontSizeSmall);
        graphics.drawText("Max Score: "+ MaxScore,(width-(width/2)+offsetXText),side/2,fontSizeSmall);
